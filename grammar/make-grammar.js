...
module.exports = function makeGrammar(options) {
  const {name, extensions = []} = options;
  const datumRules = [...commonLayer.datumRules];
  const intertokenRules = [];
  const escapeAlternatives = [];
  const symbolAlternatives = [];
  const datumRuleDefinitions = {};
  const intertokenRuleDefinitions = {};
  const contributedRules = {};
  const ruleOverrides = {};

  for (const fragment of extensions) {
    for (const rule of fragment.datumRules || []) {
      if (typeof rule === "string") {
        datumRules.push(rule);
      } else {
        const index = rule.before
          ? datumRules.indexOf(rule.before)
          : datumRules.indexOf(rule.after) + 1;
        datumRules.splice(index < 0 ? datumRules.length : index, 0, rule.name);
      }
    }
    intertokenRules.push(...(fragment.intertokenRules || []));
    escapeAlternatives.push(...(fragment.escapeAlternatives || []));
    symbolAlternatives.push(...(fragment.symbolAlternatives || []));
    Object.assign(datumRuleDefinitions, fragment.datumRuleDefinitions || {});
    Object.assign(
      intertokenRuleDefinitions,
      fragment.intertokenRuleDefinitions || {});
    Object.assign(contributedRules, fragment.rules || {});
    Object.assign(ruleOverrides, fragment.ruleOverrides || {});
  }

  const symbolRule = token(choice(
    ...symbolAlternatives,
    ...hidden_node.symbolAlternatives));
  const baseRules = {
    directive: $ => seq("#!", repeat($._intertoken), symbolRule),
  };
  const overriddenRule = (name, $) => ruleOverrides[name]
    ? ruleOverrides[name]({$, base: baseRules[name], symbol: symbolRule})
    : baseRules[name]($);
  ...
  rules: {
    ...
    directive: $ => overriddenRule("directive", $),
    ...
    _datum: $ => choice(...datumRules.map(rule => $[rule])),
    ...
    escape_sequence: _ =>
      token(
        choice(
          ...
          ...(escapeAlternatives.length
            ? escapeAlternatives
            : [extension.escape_sequence]))),

    symbol: _ => token(symbolRule),
